...
class BasicTests(unittest.TestCase):
    ...
    def test_merge_overlaps(self):
        # GIVEN
        data = [
            {'from': Timestamp('2022-01-01 04:00:00'), 'to': Timestamp('2022-01-01 14:00:00')},
            {'from': Timestamp('2022-01-01 04:00:00'), 'to': Timestamp('2022-01-01 08:00:00')},
            {'from': Timestamp('2022-01-01 08:00:00'), 'to': Timestamp('2022-01-01 16:00:00')},
            {'from': Timestamp('2022-01-02 01:00:00'), 'to': Timestamp('2022-01-02 11:00:00')},
            {'from': Timestamp('2022-02-01 04:00:00'), 'to': Timestamp('2022-02-01 08:00:00')},
            {'from': Timestamp('2022-02-01 08:00:00'), 'to': Timestamp('2022-02-01 12:00:00')},
            {'from': Timestamp('2022-02-02 08:00:00'), 'to': Timestamp('2022-02-02 16:00:00')}

        ]
        subject = pandas.DataFrame.from_dict(data)

        # WHEN
        merged_overlaps = timestamp_lib.merge_overlaps(dataframe=subject)

        # THEN
        self.assertFalse(merged_overlaps.empty)
        expected = pandas.DataFrame.from_dict([
            {'from': Timestamp('2022-01-01 04:00:00'), 'to': Timestamp('2022-01-01 16:00:00')},
            {'from': Timestamp('2022-01-02 01:00:00'), 'to': Timestamp('2022-01-02 11:00:00')},
            {'from': Timestamp('2022-02-01 04:00:00'), 'to': Timestamp('2022-02-01 12:00:00')},
            {'from': Timestamp('2022-02-02 08:00:00'), 'to': Timestamp('2022-02-02 16:00:00')}
        ])
        assert_frame_equal(expected, merged_overlaps.sort_values(['from']).reset_index(drop=True),
    ...
    def test_split_month_overflow(self):
        ...
        assert_frame_equal(expected, split_overflows.sort_values(['from']).reset_index(drop=True),
                           check_index_type=False)
